...
public interface PredictionModelMapper extends BaseMapper<PredictionModel> {
     ...
     * 查询所有启用的模型
     */
    @Select("SELECT * FROM prediction_model WHERE is_active = 1 ORDER BY model_type, model_version")
    List<PredictionModel> selectActiveModels();

    /**
     * 根据模型类型查询模型
     */
    @Select("SELECT * FROM prediction_model WHERE model_type = #{modelType} AND is_active = 1")
    ...
    PredictionModel selectByNameAndVersion(@Param("modelName") String modelName,
